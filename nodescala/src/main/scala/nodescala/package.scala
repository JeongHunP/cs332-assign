...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    /** Given a list of futures `fs`, returns the future holding the list of
      ...
      * only once all of the futures in `fs` have been completed. The values in
      * the list are in the same order as corresponding futures `fs`. If any of
      * the futures `fs` fails, the resulting future also fails.
      */
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      val promise = Promise[T]()
      fs.foreach(f => f.onComplete { case x => promise.tryComplete(x) })
      promise.future
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      ...
      f.onComplete { case _ => promise.success(cont(f)) }
      promise.future
    }
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      val promise = Promise[S]()
      f.onComplete { case res => promise.success(cont(res)) }
      promise.future
    }
